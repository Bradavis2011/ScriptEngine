...
function scriptCardHtml(s: ScriptData, index: number, total: number): string {
  const sectionsHtml = s.sections
    ...
    <div style="margin-bottom:20px;">
      <p style="font-size:11px;font-weight:700;color:#00E5FF;text-transform:uppercase;letter-spacing:0.8px;margin:0 0 6px;">${escapeHtml(sec.heading)}</p>
      <p style="margin:0 0 6px;line-height:1.7;">${escapeHtml(sec.script)}</p>
      <p style="font-size:12px;color:#666;margin:0;">B-Roll: ${escapeHtml(sec.bRollSuggestion)}</p>
    </div>`,
    )
    .join('');
  ...
  <div style="background:#141417;border:1px solid #2A2A2F;border-radius:16px;padding:24px;margin-bottom:24px;">
    ...
    <p style="font-size:18px;font-weight:700;margin:0 0 20px;line-height:1.5;">"${escapeHtml(s.coldOpen)}"</p>
    ${sectionsHtml}
    ...
    <p style="margin:0 0 20px;">${escapeHtml(s.callToAction)}</p>
    <div style="background:#0B0B0D;border-radius:12px;padding:16px;margin-bottom:16px;">
      ...
      <p style="margin:0;line-height:1.8;font-size:15px;">${escapeHtml(s.teleprompterText)}</p>
    </div>
    ...
    <p style="margin:0 0 8px;">${escapeHtml(s.caption)}</p>
...
export function renderPackReportHtml(
  ...
  const escapedNiche = escapeHtml(niche);
  const escapedTopic = escapeHtml(topic || 'general');
...
<body style="font-family:system-ui,-apple-system,sans-serif;background:#0B0B0D;color:#e5e5e7;margin:0;padding:0;">
  <div style="max-width:720px;margin:0 auto;padding:32px 24px;">
    <div style="text-align:center;margin-bottom:40px;">
      ...
      <p style="color:#888;margin:0;font-size:14px;">Niche: <strong style="color:#e5e5e7;">${escapedNiche}</strong> &nbsp;·&nbsp; Topic: <strong style="color:#e5e5e7;">${escapedTopic}</strong></p>
    </div>
