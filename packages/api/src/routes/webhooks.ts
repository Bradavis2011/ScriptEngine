...
// Revenue per product in cents (for SubscriptionEvent logging)
const REVENUE_BY_PRODUCT: Record<string, number> = {
  pro_monthly:     700,   // $7
  pro_annual:      8400,  // $84/yr → log actual annual amount
  founders_annual: 6000,  // $60/yr
};
...
router.post('/stripe', async (req: Request, res: Response) => {
  ...
  const amountPaid = session.amount_total ?? null; // in cents from Stripe
  await prisma.conciergeOrder.upsert({
    where: { stripePaymentId },
    update: {},
    create: { stripePaymentId, email, niche, topic, orderType, brief, platform, stage, audience, goals, amountPaid },
  });
...
router.post('/revenuecat', async (req: Request, res: Response) => {
  ...
  // Log SubscriptionEvent for unit economics tracking
  const revenueCents = ['INITIAL_PURCHASE', 'RENEWAL', 'REACTIVATION'].includes(type)
  ...
  const periodStart = event.period_start_at ? new Date(event.period_start_at * 1000) : undefined;
  const periodEnd   = event.period_end_at   ? new Date(event.period_end_at   * 1000) : undefined;
  ...
  // Keep lifetimeRevenueCents in sync on the Tenant record
  if (revenueCents > 0 && tenant?.id) {
    await prisma.tenant.update({
      where: { id: tenant.id },
      data: { lifetimeRevenueCents: { increment: revenueCents } },
    });
  }
